...
for kk = 1:3
  ...
  elseif kk ==2  
      filename=filename2;  % ('../Waseda_results/output_with_zero_V1/'); 
       var_Gov = 3;
        shock_seq =[1 2 3];
        shock_gov =1; 
        svar_type = 'Caldala_Kamp'
...
%      ti = 1953+(nl+4)/4:0.25:1953+(ns-2)/4;
  ...
  hold on
        ...
        L2= plot(ti, mean(multp1(i,nl+4+1:ns),2).*ones(size(multp1(i,nl+4+1:ns),2),1),'LineStyle',':','Color','r', 'LineWidth',3); 
        L3= plot(ti, multp2(i,nl+4+1:ns),'LineStyle','-','Color','b', 'LineWidth',1.5); 
        L4= plot(ti, mean(multp2(i,nl+4+1:ns),2).*ones(size(multp1(i,nl+4+1:ns),2),1),'LineStyle','--','Color','b', 'LineWidth',3); 
        L5= plot(ti, multp3(i,nl+4+1:ns),'LineStyle','-','Color','m', 'LineWidth',1.5); 
        L6= plot(ti, mean(multp3(i,nl+4+1:ns),2).*ones(size(multp1(i,nl+4+1:ns),2),1),'LineStyle','--','Color','m', 'LineWidth',3); 
...
%       ylim([ -2  3 ]);
    ...
    title(['$', char(asshock(1)), ...
           '\uparrow\ \rightarrow\ ', ...  
           char(m_asvar(j)), '$ ; Cusum For ', char(cha_imp(i))  ], 'interpreter', 'latex','FontSize',14)
  end
  
  if    type == 2
%          legend([L1,L2,L3,L4],{'Benchmark','Model 2','Model 3','Model 4'})
        legend([L1,L2,L3,L4, L5, L6],{'Gov. \psi_r =0','Mean of \psi_r =0',...
                                      'Gov. \psi_r >0','Mean of \psi_r >0',...
                                      'Tax. \psi_r >0','Mean of \psi_r >0'},'FontSize',11)
