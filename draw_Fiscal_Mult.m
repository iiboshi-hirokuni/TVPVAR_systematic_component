...
beta = 1/(1+avri); 
        ...
        %      G/G      Y/G         Di/G   Pi/G  R/g    C/G
ratio_g =[ 1/1,   4.882467,    1/1,    1,       1,       3.109974      ];%2021Sep10 corrected
...
switch  policy_type  
 case 'bench_mark'
     ...
     switch svar_type       
       ...
       case 'both_zero_sign' 
           asvar = {'g';'y';'dbt';'pi';'int';'cons'};    
          var_seq = { 'Gov';'Y'; 'Debt'; 'Pi'; 'R';'Cons' };
            var_Gov = 1;
            shock_gov = 2;
...
save_FP1 =[]; save_FP2 =[]; save_FP3 =[]; save_FP4 =[];

for j=1:nk
     h_(5000+10*j) = figure(4000+10*j);
      set( h_(5000+10*j),'Position',[20,20,900,600],'File','Gov_multi');
      
  for i = 1:size(vimp,2) 
      
      multp= squeeze(save_multp(:,j,:));
      
         save_FP1 = [save_FP1;  multp(i,nl+4+1:ns)];
...
%     plot(ti, multp_ma(nl+3+1:ns-2)+multps_ma(nl+3+1:ns-2),'LineStyle','--','Color','r', 'LineWidth',1);
    hold off    
    xlim([1952 2021]);
    ylim( [ min(multp(i,nl+4+1:ns))-0.1  max(multp(i,nl+4+1:ns))+0.1 ]  );
%     if j == 1 % g
%         ylim([ 0  4]);
%     elseif j == 2 % y
%         ylim([ -1  1]); 
...
i=1;
save(['./output/multp_',  num2str(i) '-',char(policy_type),'-' ,char(svar_type),'.xls'],...
                        'save_FP1',  '-ascii', '-tabs');  
